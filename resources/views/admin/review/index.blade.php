   ...
   <table class="table datatable">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
            <?php
$products = DB::table('products')->where('id',$allSystemInfos->productId )
->value('productName');
        ...
        <td>{{ $allSystemInfos->rating }}</td>
        <td>{{ $allSystemInfos->clientDescription }}</td>
...
@endif
        ...
        <td>
            @if($allSystemInfos->status == 0)
            <a type="button" class="btn btn-success btn-sm" href="{{ route('reviewList.edit',$allSystemInfos->id) }}">
...
@endif
            ...
            <form id="delete-form-{{ $allSystemInfos->id }}" action="{{ route('reviewList.destroy',$allSystemInfos->id) }}" method="POST" style="display: none;">
